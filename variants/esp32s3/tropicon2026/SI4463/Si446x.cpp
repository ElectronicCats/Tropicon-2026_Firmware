...
void __attribute__((weak)) SI446X_CB_RXBEGIN(int16_t rssi)
{
    (void)rssi;
}
...
void __attribute__((weak)) SI446X_CB_RXINVALID(int16_t rssi)
{
    (void)rssi;
}
...
{
    uint8_t data[] = {SI446X_CMD_CHANGE_STATE, (uint8_t)newState};
    doAPI(data, sizeof(data), NULL, 0);
}

static void clearFIFO(void)
{
    static const uint8_t clearFifo[] = {SI446X_CMD_FIFO_INFO, SI446X_FIFO_CLEAR_RX | SI446X_FIFO_CLEAR_TX};
...
{
    uint8_t data[] = {SI446X_CMD_GET_INT_STATUS, clearPH, clearMODEM, clearCHIP};
    doAPI(data, sizeof(data), buff, 8);
}
...
void Si446x_init(uint8_t cs, uint8_t sdn, int8_t irq, void *spi)
{
    ...
    _sw_miso_pin = pins[1];
    _sw_mosi_pin = pins[2];
    pinMode(_sw_sck_pin, OUTPUT);
    digitalWrite(_sw_sck_pin, LOW);
    pinMode(_sw_miso_pin, INPUT_PULLUP);
    pinMode(_sw_mosi_pin, OUTPUT);
    digitalWrite(_sw_mosi_pin, LOW);
#else
...
{
    // GET_MODEM_STATUS command (0x22), clear all pending (0xFF)
    // Response: MODEM_PEND, MODEM_STATUS, CURR_RSSI, LATCH_RSSI, ANT1_RSSI, ANT2_RSSI, AFC_FREQ_OFFSET[15:8],
    // AFC_FREQ_OFFSET[7:0]
    uint8_t data[8] = {0x22, 0xFF};
    doAPI(data, 2, data, 8);
    if (modemStatus)
        *modemStatus = data[1];
    if (currRSSI)
        *currRSSI = data[2];
    if (afcOffset)
        *afcOffset = (int16_t)((data[6] << 8) | data[7]);
}
...
void Si446x_setupWUT(uint8_t r, uint16_t m, uint8_t ldc, uint8_t config)
{
    ...
    SI446X_NO_INTERRUPT()
    {
        ...
        uint8_t props[5] = {
            (uint8_t)(doRx ? SI446X_GLOBAL_WUT_CONFIG_WUT_LDC_EN_RX
                           : 0 | doBatt << SI446X_GLOBAL_WUT_CONFIG_WUT_LBD_EN | (1 << SI446X_GLOBAL_WUT_CONFIG_WUT_EN)),
            (uint8_t)(m >> 8), (uint8_t)m, (uint8_t)(r | SI446X_LDC_MAX_PERIODS_TWO | (1 << SI446X_WUT_SLEEP)), ldc};
...
#endif
        uint8_t data[] = {SI446X_CMD_START_TX, channel, (uint8_t)(onTxFinish << 4), 0, SI446X_FIXED_LENGTH, 0, 0};
        doAPI(data, sizeof(data), NULL, 0);
#if !SI446X_FIXED_LENGTH
...
void Si446x_RX(uint8_t channel)
{
    SI446X_NO_INTERRUPT()
    {
        ...
        interrupt2(NULL, 0, 0, 0xFF);
        uint8_t data[] = {
            SI446X_CMD_START_RX,        channel, 0, 0, SI446X_FIXED_LENGTH, SI446X_STATE_NOCHANGE, (uint8_t)IDLE_STATE,
            (uint8_t)SI446X_STATE_SLEEP};
        doAPI(data, sizeof(data), NULL, 0);
    }
}
...
{
    uint8_t data[] = {SI446X_CMD_GPIO_PIN_CFG,    SI446X_GPIO_MODE_DONOTHING, SI446X_GPIO_MODE_DONOTHING,
                      SI446X_GPIO_MODE_DONOTHING, SI446X_GPIO_MODE_DONOTHING, SI446X_NIRQ_MODE_DONOTHING,
                      SI446X_SDO_MODE_DONOTHING,  SI446X_GPIO_DRV_HIGH};
    data[pin + 1] = value;
    doAPI(data, sizeof(data), NULL, 0);
}
