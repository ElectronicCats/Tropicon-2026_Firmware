...
typedef enum {
    ...
    SI446X_STATE_SLEEP =
        0x01, ///< This will never be returned since SPI activity will wake the radio into ::SI446X_STATE_SPI_ACTIVE
...
typedef enum {
    ...
    SI446X_ADDRMODE_ADDRBROADCAST ///< Match exact address or broadcast flag (broascast flag is the MSB bit of the second byte in
                                  ///< the received data)
} si446x_addrMode_t;
 ...
 * @param [sdn] Shutdown pin
 * @param [irq] Interrupt pin
 * @param [spi] Pointer to SPIClass instance (passed as void* for C compatibility)
 * @return (none)
 */
...
/**
 * @brief Get full modem status including AFC offset
 */
void Si446x_getModemStatus(uint8_t *modemStatus, uint8_t *currRSSI, int16_t *afcOffset);

/**
 * @brief Set a single radio property
 */
void Si446x_setProperty(uint16_t prop, uint8_t value);

/**
 * @brief Set multiple consecutive radio properties
 */
 ...
 * @param [len] Number of bytes to read, make sure not to read more bytes than what the FIFO has stored. The number of bytes that
 * can be read is passed in the ::SI446X_CB_RXCOMPLETE() callback.
 * @return (none)
 */
 ...
 * @param [len] Number of bytes to transmit, maximum of ::SI446X_MAX_PACKET_LEN If configured for fixed length packets then this
 * parameter is ignored and the length is set by ::SI446X_FIXED_LENGTH in Si446x_config.h
 * @param [channel] Channel to transmit data on (0 - 255)
 * @param [onTxFinish] What state to enter when the packet has finished transmitting. Usually ::SI446X_STATE_SLEEP or
 * ::SI446X_STATE_RX
 ...
 *
 * The ::SI446X_CB_LOWBATT() callback will be ran when the supply voltage drops below this value. The WUT must be configured with
 * ::Si446x_setupWUT() to enable periodically checking the battery level.
 *
 ...
 * ::SI446X_WUT_BATT Check battery voltage - If the battery voltage is below the threshold set by ::Si446x_setLowBatt() then wake
 *up the microcontroller and run the ::SI446X_CB_LOWBATT() callback.\n
 * ::SI446X_WUT_RX Enter receive mode for a length of time determinded by the ldc and r parameters (NOT SUPPORTED YET! dont use
 ...
 * @param [config] Which WUT features to enable ::SI446X_WUT_RUN ::SI446X_WUT_BATT ::SI446X_WUT_RX These can be bitwise OR'ed
 *together to enable multiple features.
 * @return (none)
 */
 ...
 *
 * If WUT is enabled then the radio will keep the internal 32KHz RC enabled with a current consumption of 740nA, otherwise the
 * current consumption will be 40nA without WUT. Sleep will fail if the radio is currently transmitting.
 *
 * @note Any SPI communications with the radio will wake the radio into ::SI446X_STATE_SPI_ACTIVE mode. ::Si446x_sleep() will need
 * to called again to put it back into sleep mode.
 *
 ...
 *
 * @note NIRQ and SDO pins should not be changed, unless you really know what you're doing. 2 of the GPIO pins (usually 0 and 1)
 * are also usually used for the RX/TX RF switch and should also be left alone.
 *
 * @param [pin] The pin, this can only take a single pin (don't use bitwise OR), see ::si446x_gpio_t
 * @param [value] The new pin mode, this can be bitwise OR'd with the ::SI446X_PIN_PULL_EN option, see ::si446x_gpio_mode_t
 * ::si446x_nirq_mode_t ::si446x_sdo_mode_t
 * @return (none)
 */
 ...
 *
 * @param [buff] Pointer to memory to place group values, if this is NULL then nothing will be dumped, just the group size is
 * returned
...
/**
 * @brief If interrupts are disabled (::SI446X_INTERRUPTS in Si446x_config.h) then this function should be called as often as
 * possible to process any events
 *
 * @return (none)
 */
 ...
 *
 * Ideally you should wrap sensitive sections with ::SI446X_NO_INTERRUPT() instead, as it automatically deals with this function
 * and ::Si446x_irq_on()
 *
 ...
 *
 * When communicating with other SPI devices on the same bus as the radio then you should wrap those sections in a
 * ::SI446X_NO_INTERRUPT() block, this will stop the Si446x interrupt from running and trying to use the bus at the same time.
 * This macro is based on the code from avr/atomic.h, and wraps the ::Si446x_irq_off() and ::Si446x_irq_on() functions instead of
 * messing with global interrupts. It is safe to return, break or continue inside an ::SI446X_NO_INTERRUPT() block.
 *
 * Example:
 *
 * Si446x_RX(63);\n
...
#define SI446X_NO_INTERRUPT()                                                                                                    \
    for (uint8_t si446x_irq __attribute__((__cleanup__(_Si446x_iRestore))) = Si446x_irq_off(), si446x_tmp = 1; si446x_tmp;       \
         si446x_tmp = 0)
